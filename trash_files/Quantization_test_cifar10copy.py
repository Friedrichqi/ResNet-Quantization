...
for idx, resnet_model in enumerate(resnet_models):
    sys.stdout = open(f"ptq_resnet{idx_list[idx]}_cifar10_bn_layerwise.out", "w")

    model = resnet_model()
    model.fc = nn.Linear(model.fc.in_features, num_classes)
    state_dict = torch.load(f'/data/home/qyjh/Quantization_evaluation/Resnet_Test/model/Resnet{idx_list[idx]}_CIFAR10.pt')
