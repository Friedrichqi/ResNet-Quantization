...
for idx, resnet_model in enumerate(resnet_models):
    sys.stdout = open(f"ptq_resnet{idx_list[idx]}_cifar100_bn_layerwise.out", "w")

    model = resnet_model()
    model.fc = nn.Linear(model.fc.in_features, num_classes)
    state_dict = torch.load(f'/data/home/qyjh/Quantization_evaluation/Resnet_Test/model/Resnet{idx_list[idx]}_CIFAR100.pt')
    ...
    for idx_layer, layer in enumerate(layer_list):
        for bit in range(8, 0, -1):
            ...
            print(f'Accuracy of the bn layer{idx_layer+1} quantized model ({bit}-bit): {accuracy:.2f}%')
